...
func apply_dialogue_line() -> void:
	...
	var path = "res://data/character/" + dialogue_line.character.to_lower() + ".tres"
	if ResourceLoader.exists(path):
		var character_data:CharacterData = ResourceLoader.load(path)
		character_label.modulate = character_data.theme_color
		character_label.text = character_data.character_name
		
	if dialogue_line.character.to_lower() == "it":
...
func _on_dialogue_label_spoke(letter: String, letter_index: int, speed: float) -> void:
	type_sound.pitch_scale = randf_range(0.7, 1.3)
	type_sound.play()
